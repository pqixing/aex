...
class XBuilderDialog(val action: XBuilderAction, e: AnActionEvent) : XModuleDialog(e) {
    ...
    fun buildTasks(newTask: String? = null): List<String> {
        val buildTasks = "BUILD_TASK".getSp(null, project)?.toString() ?: "assembleDebug,assembleRelease"
        val tasks = buildTasks.split(",").filter { it.isNotEmpty() }.toMutableList()
        if (newTask != null) {
            tasks.remove(newTask)
            tasks.add(0, newTask)
            "BUILD_TASK".putSp(tasks.joinToString(","), project)
        }
        return tasks
    }
