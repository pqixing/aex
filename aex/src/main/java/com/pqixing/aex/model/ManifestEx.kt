...
open class ManifestEx(dir: String) : ManifestX(dir), IManifestEx {
    ...
    fun afterEvaluated(set: XSetting) {
        ...
        val encodes =
            (mavens.map { it.psw } + gits.map { it.psw }).filter { !it.startsWith("sk:") }.map { it to ("sk:" + it.base64Encode().reversed()) }
        if (encodes.isNotEmpty()) {
            set.println("Password Encode Suggestion : ${encodes.joinToString(",") { "${it.first} -> ${it.second}" }}")
        }
    }
